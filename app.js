class App {
  ...
  openTooltip(event) {
    ...
    const vertical = -25 + window.scrollY;

    this.$colorTooltip.style.transform = `translate(${horizontal}px, ${vertical}px)`;
    this.$colorTooltip.style.display = 'flex';
    this.$colorTooltip.style.background = "blue";
  }
    ...
    this.$notes.innerHTML = this.notes.map(note => `
      <div style="background: ${note.color};" class="note" data-id="${note.id}">
        ...
        <div class="toolbar-container">
          <div class="toolbar">
            <img src="https://icon.now.sh/palette" alt="" class="toolbar-color" data-id='${note.id}'>
            <img src="https://icon.now.sh/delete" alt="" class="toolbar-delete" data-id='${note.id}'>
          </div >
        </div >
      </div >
  `).join("");
